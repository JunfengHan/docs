...
上面的操作是不是有点太详细（啰嗦）了 😊 ，为了不给某些新来的小伙伴造成阅读困难，这里感觉有必要说一下，后面说的源码是指 github 拉取的 v17.0.1。
...
createContainer 方法返回值是 createFiberRoot 方法的返回值；
...
> ⚠️ 有个需要说明的地方是，上面具体执行 DOM 插入、更新、删除的分别是 commitPlacement、commitWork、commitDeletion 方法，按照普通的模块引入去找找这个方法具体做了什么。
...
commitMutationEffects 方法从 ReactFiberCommitWork.old.js 模块中倒入了 commitPlacement 等。
...
ReactFiberCommitWork.old.js 模块中倒入了 commitPlacement 方法。
...
省略判断和递归调用的部分，主要执行的是 insertBefore、appendChild 这两个方法用于**DOM 操作**。

乱找一通后发现，appendChild、insertBefore 方法来自于 ReactFiberHostConfig.js

仔细看看 🧐 ReactFiberHostConfig.js 里有什么吧！
...
你没看错，ReactFiberHostConfig.js 除去注释只有 2 行代码，**并没有我们要找的**appendChild、insertBefore 方法。

![问号脸](../_media/question_mark.jpeg)

刚看到我也是一脸问号，本以为自己找错文件了，仔细看看注释，又跑去 Google 了一番，发现自己 too young 啊。
...
终于找到了 insertBefore、appendChild，就是普通的 DOM 操作，调用了原生的 DOM 操作方法。
